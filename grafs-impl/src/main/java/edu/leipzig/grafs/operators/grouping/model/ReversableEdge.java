...
public class ReversableEdge extends Edge {
   ...
   */
  private ReversableEdge(boolean reverse, GradoopId id, String label, GradoopId sourceId, GradoopId targetId,
  ...
  public static ReversableEdge create(Edge e, boolean reverse) {
    ReversableEdge revEdge;
    if (reverse) {
      revEdge = new ReversableEdge(reverse, e.getId(), e.getLabel(), e.getTargetId(), e.getSourceId(),
          e.getProperties(),
          e.getGraphIds());
    } else {
      revEdge = new ReversableEdge(reverse, e.getId(), e.getLabel(), e.getSourceId(), e.getTargetId(),
          e.getProperties(),
          e.getGraphIds());
    }
    return revEdge;
