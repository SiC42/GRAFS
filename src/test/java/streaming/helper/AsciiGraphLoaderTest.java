...
class AsciiGraphLoaderTest {
    ...
    private Collection<Edge> loadFromString_cycleGraph_init(){
        return AsciiGraphLoader.loadFromString( "[(v1:TestVertex)-[e1:TestEdge]->(v2:TestVertex) (v2:TestVertex)-[e2:TestEdge2]->(v1:TestVertex)]");
    }
    ...
    private Collection<Edge> loadFromString_oneEdgeWithInfo_init() {
        return AsciiGraphLoader.loadFromString("[(alice:User {name: 'Alice'})-[e1:TestEdge {type: 'relationship'}]->(bob:User {name: 'Bob'})]");
    }
